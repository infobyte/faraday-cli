...
def test_workspaces(ok_configuration_file):
    ...
    cli_execution = runner.invoke(cli, ["workspace", "-a", "create", "-n", workspace_name])
    ...
    assert cli_execution.exit_code == 0
    cli_execution = runner.invoke(cli, ["workspace", "-a", "delete", "-n", workspace_name])
    assert cli_execution.exit_code == 0
...
def test_select_workspace(ok_configuration_file):
    ...
    assert cli_execution.exit_code == 0
    assert f"Created workspace: {workspace_name}" in cli_execution.output
    cli_execution = runner.invoke(cli, ["workspace", "-a", "select", "-n", workspace_name])
    assert cli_execution.exit_code == 0
