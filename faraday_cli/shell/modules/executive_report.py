...
class ExecutiveReportsCommands(cmd2.CommandSet):
    ...
    list_templates_parser.add_argument("-w", "--workspace-name", type=str, help="Workspace name")
    ...
    def list_executive_reports_templates(self, args):
        ...
        templates_data = self._cmd.api_client.get_executive_report_templates(workspace_name)
        data = []
    ...
    report_parser.add_argument("-w", "--workspaces-names", type=str, help="Workspace name")
    report_parser.add_argument("-t", "--template", type=str, help="Template to use", required=False)
    report_parser.add_argument("--title", type=str, help="Report title", default="")
    report_parser.add_argument("--summary", type=str, help="Report summary", default="")
    report_parser.add_argument("--enterprise", type=str, help="Enterprise name", default="")
    report_parser.add_argument("--conclusions", type=str, help="Report conclusions", default="")
    ...
    report_parser.add_argument("--scope", type=str, help="Report scope", default="")
    report_parser.add_argument("--objectives", type=str, help="Report objectives", default="")
    report_parser.add_argument("--confirmed", action="store_true", help="Confirmed vulnerabilities")
    report_parser.add_argument(
        "--severity",
        type=str,
    ...
    report_parser.add_argument("-d", "--destination", type=str, help="Report destination")
    ...
    def generate_executive_report(self, args):
        ...
        def get_report(report_data, _output):
            report_id = self._cmd.api_client.generate_executive_report(workspaces_names, report_data)
            report_status = "processing"
            while report_status == "processing":
                time.sleep(2)
                report_status = self._cmd.api_client.get_executive_report_status(report_id)

            else:
                if report_status == "created":
                    download_response = self._cmd.api_client.download_executive_report(report_id)
        ...
        if not args.workspaces_names:
            ...
            workspaces_names = [name.strip() for name in args.workspaces_names.split(",")]

        templates_data = self._cmd.api_client.get_executive_report_templates(workspaces_names)
        report_name = "_".join(filter(None, [*workspaces_names, args.title, args.enterprise]))
        if not args.template:
            data = []
            template_index = 0
            templates_choices = {}
            for _template in sorted(templates_data["items"], key=lambda x: x[1]):
                template_index += 1
            ...
            templates = {template[1]: template[0] for template in templates_data["items"]}
